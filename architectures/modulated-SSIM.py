...
# -*- coding: utf-8 -*-
"""
Plantilla xarxes neurals AE amb coll d'ampolla modulat 2023011002
V2.2
...
Aquest script està basat en CSMR2023011001 v6.0.
...
-Separar logs per cada valor de lambda per a seguiment a tensorboard amb nombre variable de inputs lambda.
...
class CSMR2023011001_v6_0(tf.keras.Model):
  """Main model class."""
...
def train(args):
  ...
  model = CSMR2023011001_v6_0(args.lmbda, args.num_filters, args.bands, args.num_scales, args.scale_min, args.scale_max)
  model.compile(
  ...
  for input_file in inputs:
      ...
      if args.bitrate == None:
          ...
          while iteration < 101: #limited to 100 iterations to avoid infinite loops.
            print('Iteration '+str(iteration)+' at between '+str(float(bps_min))+'bps and '+str(float(bps_max))+'bps.')
            iteration += 1
...
def parse_args(argv):
  ...
  parser.add_argument(
      "--model_path", default="CSMR2023011001_v6_0",
  ...
  train_cmd.add_argument(
      "--train_path", default="/tmp/CSMR2023011001_v6_0",
