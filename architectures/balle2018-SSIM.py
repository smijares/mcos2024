...
# -*- coding: utf-8 -*-
"""
Ballé 2018 architecture with range-adaptive normalisation for SSIM
V1.0
Sebastià Mijares i Verdú - GICI, UAB
sebastia.mijares@uab.cat

Script to train and run compression models with a fixed rate-distortion tradeoff, optimised for SSIM.
...
class HyperSynthesisTransform(tf.keras.Sequential):
  ...
  def __init__(self, num_filters, num_filters_final):
    ...
    self.add(tfc.SignalConv2D(
        ...
        activation=tf.nn.relu))
    self.add(tfc.SignalConv2D(
        num_filters_final, (3, 3), name="layer_2", corr=False, strides_up=1,
        padding="same_zeros", use_bias=True, kernel_parameter="variable",
        activation=None))


class NLTC2023020901_v1_0(tf.keras.Model):
  """Main model class."""
  ...
  def __init__(self, lmbda, num_filters, num_scales, scale_min, scale_max, bands):
    ...
    self.hyper_analysis_transform = HyperAnalysisTransform(num_filters[0])
    self.hyper_synthesis_transform = HyperSynthesisTransform(num_filters[0],num_filters[1])
    self.hyperprior = tfc.NoisyDeepFactorized(batch_shape=(num_filters[0],))
  ...
  def call(self, x, training):
    ...
    ssim = tf.image.ssim(x, x_hat, 65535)
  ...
  def compile(self, **kwargs):
    ...
    self.mse = tf.keras.metrics.Mean(name="SSIM")
...
def store_latent(args):
  ...
  for input_file in inputs:
      if args.height == None or args.width == None or args.bands == None:
          ...
          x = read_raw(input_file,args.height,args.width,args.bands,args.endianess)
          path = input_file
      y, filters = model.store_latent(x)
      
      write_raw(path+'_latent.'+str(int(filters))+'_'+str(int(width)//16)+'_'+str(int(height)//16)+'_6_0_0.raw', y)
...
def parse_args(argv):
  ...
  parser.add_argument(
      "--model_path", default="NLTC2023020901_v1_0",
  ...
  train_cmd.add_argument(
      "--train_path", default="/tmp/NLTC2023020901_v1_0",
  ...
  compress_cmd.add_argument(
        "--keep_latent", action="store_true",
        help="When selected, stores the latent representation of the image with the current model as it is produced by the learned main transform.")
