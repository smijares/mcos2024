...
# -*- coding: utf-8 -*-
"""
MCOS2023 modulated architecture for MSE
V4.0
Sebastià Mijares i Verdú - GICI, UAB
sebastia.mijares@uab.cat

Script to train and run compression models with a modulated rate-distortion tradeoff, optimised for MSE.
...
class CSMR2023011001_v4_0(tf.keras.Model):
  """Main model class."""
  ...
  def __init__(self, lmbda, num_filters, bands, num_scales, scale_min, scale_max):
    ...
    self.modulating_transform = ModulatingTransform(64, 1, num_filters[1], max(lmbda))
  ...
  def compress(self, x, lmda):
    ...
    mod = self.modulating_transform(tf.expand_dims(tf.expand_dims(tf.expand_dims(tf.expand_dims(lmda,0),0),0),0))
    y = mod*self.analysis_transform((x-m)/(M-m+1))
    z = self.hyper_analysis_transform(abs(y))
    ...
    return string, side_string, x_shape, y_shape, z_shape, L, tf.as_string([tf.squeeze(m)]), tf.as_string([tf.squeeze(M)]), mod
...
def train(args):
  ...
  model = CSMR2023011001_v4_0(args.lmbda, args.num_filters, args.bands, args.num_scales, args.scale_min, args.scale_max)
  model.compile(
...
def compress(args):
  ...
  for input_file in inputs:
      ...
      string, side_string, x_shape, y_shape, z_shape, L, m_string, M_string, mod = model.compress(x, tf.constant([args.quality]))
...
def parse_args(argv):
  ...
  parser.add_argument(
      "--model_path", default="CSMR2023011001_v4_0",
  ...
  train_cmd.add_argument(
      "--train_path", default="/tmp/CSMR2023011001_v4_0",
  ...
  compress_cmd.add_argument("--quality",
                            ...
                            help="Lambda value regulating the rate-distortion tradeoff. A higher lambda should yield higher quality and rate images."
                            )
